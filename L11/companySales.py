...
# 1. crearea unui fisier CSV cu date de vanzari
date_vanzari = {
...
# 2. incarcarea CSV-ului pentru analiza
df = pd.read_csv('vanzari.csv')

# 3. conversia coloanei data vanzarii in format datetime pentru a putea grupa si filtra
df['data vanzarii'] = pd.to_datetime(df['data vanzarii'], dayfirst=True)

# 4. calcularea venitului pentru fiecare rand
df['venit'] = df['cantitate vanduta'] * df['pret']

# 5. adaugarea unei coloane pentru luna si anul vanzarii
df['an_luna'] = df['data vanzarii'].dt.to_period('M')

# 6. cele mai vandute produse pe luna
...
# 7. venitul total pe fiecare produs
...
# 8. filtrarea vanzarilor intre doua date
